...
| getRTCToken             | Function                | No       | The method for getting the Agora token to use RTC. getRTCToken({channel: string, username: string}) => {accessToken: string}                                                                                   |
| getIdMap                | Function                | No       | The method for getting the mapping between the Agora Chat user ID and the user ID you expect to display on the Web UI, when RTC is used. getIdMap({userId: string, channel: string})=> {[key: string]: string} |
